...
const commitRes = await api('POST', `/repos/${REPO}/git/commits`, {
  message: 'fix: 顶部科目筛选按钮恢复为只筛选（不直接进入刷题）',
  tree: treeRes.data.sha, parents: [baseSha],
});
const updateRes = await api('PATCH', `/repos/${REPO}/git/refs/heads/main`, { sha: commitRes.data.sha });
console.log('Status:', updateRes.status, 'Commit:', commitRes.data?.sha?.slice(0,12));
console.log('✅ Pushed');
