...
def migrate_db():
    ...
    if 'questions' in table_names:
        ...
        for row in all_qs:
            ...
            # 去掉题干末尾嵌入的选项文本（如 "来源之一是B. xxxC. xxxD. xxx"）
            new_qtext = re.sub(r'[A-D][\.。、]\s*\S.*', '', new_qtext)
...
def clean_question_text(raw_text):
    """清洗题目原始文本，去掉题号、答案、嵌入选项、题型标记等干扰信息"""
    text = raw_text.strip()
    ...
    # ===== 关键修复：去掉题干末尾嵌入的连续选项文本 =====
    # 例如："来源之一是B. 英国古典政治经济学C. 英法想社会主义D. 现代西方哲学"
    # 匹配 [A-D][.] + 空格 + 任意非空内容 到 行尾
    text = re.sub(r'[A-D][\.、]\s*\S.*', '', text)

    # 不含/不包括等关键词后的答案泄露
    text = re.sub(r'(不含|不包括|不属于|没有|排除)\s*[ABCD][\.、，）)]\s*\S*', r'（  ）)', text)
    text = re.sub(r"(不含|不包括|不属于|没有|排除)\s*[ABCD][\.、，）)]\s*\S*", r"（  ）", text)
    # 空括号
...
def search_questions():
    ...
    if sub:
        # Support both subject_name and Subject.name matching
        query = query.filter((Question.subject_name == sub) | (Question.subject_id == db.session.query(Subject.id).filter(Subject.name == sub).scalar_subquery()))
    if qt:
