...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getAnnouncements(), getAdminQuestions(),
    ]).then(([u, e, s, ta, tu, st, ann, aq]) => {
      setUsers(u); setExams(e); setStats(s); setTotalAnswers(ta); setTotalUsers(tu);
      setSettings(st); setAnnouncements(ann); setAdminQuestions(aq);
  ...
  const sectionStyle = { background: 'white', borderRadius: 12, padding: 24, marginBottom: 16, border: '1px solid #eee' };

  return (
    <div className="stats-page" style={{ maxWidth: 960 }}>
          ...
          {tab === 'dashboard' && (
            <div>
              <div className="stats-grid">
                ...
                <div className="stat-card"><div className="stat-card-value" style={{ color: '#8b5cf6' }}>{exams.length}</div><div className="stat-card-label">考试场次</div></div>
              </div>
              ...
              <div style={sectionStyle}>
                <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 16 }}>📝 最近考试</h3>
                {exams.slice(0, 5).length === 0 ? <p style={{ color: '#9ca3af', textAlign: 'center', padding: 20 }}>暂无</p> : exams.slice(0, 5).map(ex => {
                  const d = new Date(ex.completed_at);
                  return (
                    <div key={ex.id} className="category-stat-item">
                      <div className="category-stat-info">
                        <div className="category-stat-name">{ex.category || '全科'} · {ex.total_questions}题 <span style={{ color: '#9ca3af', fontSize: 13 }}>{d.getMonth()+1}/{d.getDate()}</span></div>
                        <div className="category-stat-bar" style={{ height: 6 }}>
                          <div className="category-stat-fill" style={{ width: `${ex.score}%`, background: ex.score >= 80 ? 'linear-gradient(90deg,#10b981,#34d399)' : ex.score >= 60 ? 'linear-gradient(90deg,#f59e0b,#fbbf24)' : 'linear-gradient(90deg,#ef4444,#f87171)' }} />
                        </div>
                      </div>
                      <span style={{ fontWeight: 700, color: ex.score >= 80 ? '#10b981' : ex.score >= 60 ? '#f59e0b' : '#ef4444' }}>{ex.score}分</span>
                    </div>
                  );
                })}
              </div>
          ...
          {tab === 'questions' && (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                ...
                <button className="btn btn-primary" style={{ padding: '8px 20px', fontSize: 14 }} onClick={() => { setQForm({ editing: null, category: 'javascript', difficulty: 1, type: 'single', title: '', options: ['', '', '', ''], answer: '0', explanation: '' }); setQFormVisible(true); }}>➕ 新增题目</button>
              </div>
              {qFormVisible && qForm && (
                      ...
                      <div key={q.id} style={{ padding: '14px 20px', borderBottom: '1px solid #f3f4f6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                            <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 8, background: '#eef2ff', color: '#4f46e5' }}>{categories.find(c => c.id === q.category)?.name || q.category}</span>
                            ...
                            {!q.is_active && <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 8, background: '#fef9c3', color: '#ca8a04' }}>已禁用</span>}
                          </div>
                        </div>
